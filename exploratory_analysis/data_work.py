...
class DataWork:
    ...
    def load_data(self):
        ...
        try:
            ...
            self.df_usadas = pd.read_csv(
                'exploratory_analysis/notebooks_usadas.csv')
        except FileNotFoundError as e:
            print("Archivo no encontrado", e)
            print("Ejecute el módulo scrap y vuelva a repetir este proceso")

    def format_data_type(self):
        # * Convertir a númerics los datos de precios
        self.df_nuevas['precio'] = pd.to_numeric(self.df_nuevas['precio'],
                                                 errors='coerce')
    ...
    def data_analystic(self):
        ...
        self.df_nuevas_sorted = self.df_nuevas.sort_values('precio',
                                                           ascending=False)
        self.df_usadas_sorted = self.df_usadas.sort_values('precio',
                                                           ascending=False)
        ...
        self.precio_nuevas_percentiles = self.df_nuevas['precio'].quantile(
            [0.25, 0.50, 0.75])
